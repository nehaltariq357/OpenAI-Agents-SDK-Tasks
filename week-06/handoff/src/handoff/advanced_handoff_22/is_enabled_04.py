...
weather_agent: Agent = Agent(
    ...
    model=llm_model,
    handoffs=[handoff(agent=news_agent, is_enabled=news_region("us-east-1")), planner_agent] # palnner agent is used here for fallback, if newsagent is not running due to condition false, then planner agent runs
)
def is_enabled():
    res = Runner.run_sync(weather_agent,"Check if there's any news about OpenAI after GPT-5 launch - also what's the weather SF?", 
